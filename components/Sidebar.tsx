  ...
  const modes: { id: StudioMode; label: string; icon: React.ElementType; desc: string; color: string; track?: string }[] = [
    {
      ...
      icon: Bot,
      desc: 'Tool calling, function execution & Mermaid flowcharts',
      color: 'text-cyan-400',
    ...
    },
    {
      ...
      icon: Stethoscope,
      desc: 'Superbug resistance, clinical triage & stewardship',
      color: 'text-rose-400',
    ...
    },
    {
      ...
      icon: Sprout,
      desc: 'Crop foliage triage, soil health & civic repair',
    ...
    },
    {
      ...
      icon: Lightbulb,
      desc: 'Intuitive mental models & analogies',
    ...
    },
    {
      ...
      icon: Code2,
      desc: 'Refactor, Big-O analysis & vulnerability triage',
      color: 'text-blue-400',
    },
    {
      id: 'quizmaster',
      label: 'Quiz Master',
      icon: HelpCircle,
      desc: 'Self-assessment multiple choice quizzes',
      color: 'text-indigo-400',
    },
  ];

  return (
    <aside className="w-full lg:w-84 glass-panel rounded-2xl p-4 flex flex-col gap-6 shrink-0 border border-white/10">
      ...
      <div className="p-3 rounded-xl bg-gradient-to-r from-purple-950/60 via-blue-950/60 to-cyan-950/60 border border-purple-500/30">
        <div className="flex items-center gap-1.5 text-xs font-bold text-purple-300 mb-1">
          ...
          <span>Competition Winning Architecture</span>
        </div>
        <p className="text-[11px] text-slate-300 leading-snug">
          Dual-Track Ready: Covers both <strong className="text-cyan-300">Autonomous Agent</strong> & <strong className="text-emerald-300">GenAI for Good</strong> tracks.
        </p>
      </div>
      ...
      {/* Session History */}
      <div className="flex-1 flex flex-col min-h-[160px] max-h-[260px]">
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-1.5">
          {history.length === 0 ? (
            <div className="text-center py-5 text-xs text-slate-500 italic bg-slate-950/30 rounded-xl border border-white/5 px-2">
              No recent sessions. Select an engine above to start!
            </div>
          ) : (
            history.map((item) => (
              <button
